...
/* * * * * * * Script utilisé pour les graphiques simples * * * * * * */
...
function addFlag(containerFlag, getLastDataList)
{
	...
	/* Ajout du flag au graphique Highcharts */
	...
	$(dataList).on('change', function() {
		...
		addAjaxQueryOnDataList(dataList, idFlag, idYAxis, true);
	...
	$(parameterDataList).on('change', function() {
		idYAxis = parseInt($(this).attr('id').match(/\d+/g)[0], 10);
		idFlag = parseInt($(this).attr('id').match(/\d+/g)[1], 10);
		//chart.yAxis[idYAxisChart-1].series[(orderSeries -1)].setData();
		setSerie(idFlag, $(dataList).val(), $(this).val(), idYAxis, true);
...
/*function style_Flag(styleFlag,i)
{
	...
	console.log(select);
	styleFlag.val(select).css('font-family', select);
...
/* * * * *   Fonction permettant la mise à jours automatique des champs titleSerie et unitSerie en fonction du parametre   * * * * */
