...
function addOneSerie(containerSerie, getLastDataList)
{
	...
	//var invertedChart = $('input#mc_chartbundle_chart_invertedChart'); -> propriété non dispo pour les graphiques temporels
	...
	/* Ajout de la série au graphique HighStock */
	...
	$(dataList).on('change', function() {
		...
		addAjaxQueryOnDataList(dataList, idSerie, (idYAxis+2), false); // +2 car les graphiques HighStock dispose déjà de 2 axes Y par défaut
	...
	$(parameterDataList).on('change', function() {
		...
		setSerie(idSerie, $(dataList).val(), $(this).val(), (idYAxis+2), false);
		sortSeries(chart.yAxis[idYAxis+1].series);
